        ...
        if (mysqli_query($link, $sql)) {
            ...
            foreach ($_FILES["PRO_ressources"]["error"] as $key => $error) {
                if ($error == UPLOAD_ERR_OK) {
                    ...
                    $url = "uploads/$name";
                    $minioClient->putObject($name,$tmp_name);
/*                    move_uploaded_file($tmp_name, $url);*/

                    $sql = "INSERT INTO ressources (RE_type,RE_url, RE_nom,PRO_id) VALUES ('minio_obj','$url', '$name' ,$PRO_id)";
    ...
    case 'supprimer_ressource':
        if (isset($_POST['RE_id'])) {
            ...
            if (mysqli_num_rows($res) > 0) {
                ...
                if (mysqli_query($link, $sql)) {
                    if (file_exists($ressource['RE_url']) && "img" == $ressource['RE_type']) {
                        unlink($ressource['RE_url']);
                    }elseif("minio_obj" == $ressource['RE_type']){
                        $minioClient->deleteObject($ressource['RE_nom']);
                    }
                    echo 'OK';
                } else {
                    echo 'NOK';
